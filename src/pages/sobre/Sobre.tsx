function Sobre() {
    ...
    return (
        <div className="flex flex-col items-center py-10 px-5 bg-[#efefef]">
            ...
            <h1 className="text-4xl font-bold xl:py-10 pt-15 pb-8">Colaboradores</h1>
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-20 pb-5">
                {colaboradores.map((colaborador, i) => (
                    <div key={i} className="flex flex-col items-center bg-white shadow-lg rounded-lg p-6 w-72 transition-transform hover:scale-105">
                        <img src={colaborador.imagem} alt={`Foto de ${colaborador.nome}`} className="w-32 h-32 rounded-full object-cover border-2 border-[#f1af09]"/>
                        <h2 className="text-xl font-semibold mt-4">{colaborador.nome}</h2>
                        <   p className="text-gray-600">{colaborador.cargo}</p>
                        <a href={`https://github.com/${colaborador.github}`} target="_blank" className="mt-2 text-[#f1af09] hover:underline">
                            @{colaborador.github}
                        </a>
                    </div>
                ))}
